<?php
/**
 * Jaws Upgrade Stage - From 2.0.0 to 2.1.0
 *
 ...
 */
class Upgrader_200To210 extends JawsUpgrader
{
    /**
    ...
    function Display()
    {
        ...
        $tpl->Load('display.html', 'stages/200To210/templates');
        $tpl->SetBlock('200To210');

        $tpl->setVariable('lbl_info',  $this::t('VER_INFO', '2.0.0', '2.1.0'));
        $tpl->setVariable('lbl_notes', $this::t('VER_NOTES'));
        $tpl->SetVariable('next',      Jaws::t('NEXT'));

        $tpl->ParseBlock('200To210');
        return $tpl->Get();
    }
    ...
    function Run()
    {
        ...
        foreach ($schema_array as $old => $new) {
            ...
            $result = Jaws_DB::getInstance()->installSchema($new_schema, $variables, $old_schema);
