<?php
/**
 * Jaws Upgrade Stage - From 0.8.18 to 0.8.19
 *
 ...
 */
class Upgrader_0818To0819 extends JawsUpgraderStage
{
    /**
    ...
    function Display()
    {
        $tpl = new Jaws_Template(UPGRADE_PATH  . 'stages/0818To0819/templates/');
        $tpl->Load('display.html', false, false);
        $tpl->SetBlock('0818To0819');

        $tpl->setVariable('lbl_info',  _t('UPGRADE_VER_INFO', '0.8.18', '0.8.19'));
        $tpl->setVariable('lbl_notes', _t('UPGRADE_VER_NOTES'));
        $tpl->SetVariable('next',      _t('GLOBAL_NEXT'));

        $tpl->ParseBlock('0818To0819');
        return $tpl->Get();
    }
    ...
    function Run()
    {
        ...
        _log(JAWS_LOG_DEBUG,"Cleaning previous registry cache data files - step 0.8.18->0.8.19");
        ...
        _log(JAWS_LOG_DEBUG,"Cleaning previous acl cache data files - step 0.8.18->0.8.19");
