...
class Jaws_Gravatar
{
    function GetGravatar($email = '', $size = 48)
    {
        ...
        if (empty($email) || $GLOBALS['app']->Registry->Get('use_gravatar', 'Settings', JAWS_COMPONENT_GADGET) == 'no') {
            return $defaultImage;
        }

        $id = md5($email);
        $rating = $GLOBALS['app']->Registry->Get('gravatar_rating', 'Settings', JAWS_COMPONENT_GADGET);
