    ...
    function UpdateProperties($unit, $update_period, $date_format, $api_key)
    {
        ...
        foreach ($res as $r) {
            if (Jaws_Error::IsError($r) || !$r) {
                $this->app->session->PushLastResponse(_t('WEATHER_ERROR_PROPERTIES_NOT_UPDATED'), RESPONSE_ERROR);
                return new Jaws_Error(_t('WEATHER_ERROR_PROPERTIES_NOT_UPDATED'));
            }
        }

        $this->app->session->PushLastResponse(_t('WEATHER_PROPERTIES_UPDATED'), RESPONSE_NOTICE);
        return true;
    }
}