...
class Glossary_Model_Admin_Term extends Jaws_Gadget_Model
{
    ...
    function DeleteTerm($id)
    {
        ...
        if (Jaws_Error::IsError($result)) {
            $this->app->session->PushLastResponse(_t('GLOSSARY_ERROR_TERM_NOT_DELETED'), RESPONSE_ERROR);
            return new Jaws_Error(_t('GLOSSARY_ERROR_TERM_NOT_DELETED'));
        }

        $this->app->session->PushLastResponse(_t('GLOSSARY_TERM_DELETED'), RESPONSE_NOTICE);
        return true;
    }
    ...
    function UpdateTerm($id, $term, $fast_url, $desc)
    {
        ...
        if (Jaws_Error::IsError($result)) {
            $this->app->session->PushLastResponse(_t('GLOSSARY_ERROR_TERM_NOT_UPDATED'), RESPONSE_ERROR);
            return new Jaws_Error(_t('GLOSSARY_ERROR_TERM_NOT_UPDATED'));
        }

        $this->app->session->PushLastResponse(_t('GLOSSARY_TERM_UPDATED'), RESPONSE_NOTICE);
        return true;
    }
    ...
    function NewTerm($term, $fast_url, $desc)
    {
        ...
        if (Jaws_Error::IsError($result)) {
            $this->app->session->PushLastResponse(_t('GLOSSARY_ERROR_TERM_NOT_CREATED'), RESPONSE_ERROR);
            return new Jaws_Error(_t('GLOSSARY_ERROR_TERM_NOT_CREATED'));
        }

        $this->app->session->PushLastResponse(_t('GLOSSARY_TERM_ADDED'), RESPONSE_NOTICE);
