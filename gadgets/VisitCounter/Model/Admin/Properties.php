...
class VisitCounter_Model_Admin_Properties extends Jaws_Gadget_Model
{
    ...
    function UpdateProperties($counters, $numdays, $type, $mode, $custom_text='')
    {
        ...
        if ($rs1 && $rs2 && $rs3 && $rs4 && $rs5) {
            $this->gadget->session->push(_t('VISITCOUNTER_PROPERTIES_UPDATED'), RESPONSE_NOTICE);
            return true;
        }

        $this->gadget->session->push(_t('VISITCOUNTER_ERROR_PROPERTIES_UPDATED'), RESPONSE_ERROR);
