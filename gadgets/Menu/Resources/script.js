...
function saveMenus()
{
    if (currentAction == 'Groups') {
        if (!$('#title').val()) {
            alert(jaws.gadgets.Menu.incompleteFields);
            return false;
        }
        cacheMenuForm = null;
...
function addGroup()
{
    ...
    $('#edit_area span').first().html(jaws.gadgets.Menu.addGroupTitle);
    selectedGroup = null;
...
function addMenu(gid, pid)
{
    ...
    if (pid == 0) {
        $('#edit_area').find('span').first().html(
            jaws.gadgets.Menu.addMenuTitle + ' - ' + $('#group_'+gid).find('a').first().html()
        );
    } else {
        $('#edit_area').find('span').first().html(
            jaws.gadgets.Menu.addMenuTitle + ' - ' + $('#group_'+gid).find('a').first().html() +
...
function editGroup(gid)
{
    ...
    $('#edit_area span').first().html(jaws.gadgets.Menu.editGroupTitle + ' - ' + $('#group_'+gid + ' a').first().html());
    $('#btn_cancel').css('display', 'inline');
...
function editMenu(mid)
{
    ...
    $('#edit_area span').first().html(jaws.gadgets.Menu.editMenuTitle + ' - ' + $('#menu_'+mid + ' a').first().html());
    $('#btn_cancel').css('display', 'inline');
    $('#btn_del').css('display', 'inline');
    $('#btn_save').css('display', 'inline');
    $('#btn_add').css('display', 'none');
    ...
    if (!menuInfo['image']) {
        ...
        $('#image').attr('src', jaws.gadgets.Menu.base_script + '?gadget=Menu&action=LoadImage&id=' + menuInfo['id'] + '&' + $.now());
    }
}
...
function delMenus()
{
    if (currentAction == 'Groups') {
        ...
        var msg = jaws.gadgets.Menu.confirmGroupDelete;
        ...
        var msg = jaws.gadgets.Menu.confirmMenuDelete;
