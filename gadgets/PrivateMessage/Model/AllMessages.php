 ...
 */
class PrivateMessage_Model_AllMessages extends Jaws_Gadget_Model
{
    /**
     * Get All Messages
     *
     ...
     */
    function GetAllMessages($user, $filters = null, $limit = 0, $offset = null)
    {
        $table = Jaws_ORM::getInstance()->table('pm_messages', 'message');
        $table->select(
            'message.id:integer','message.subject', 'message.body', 'message.insert_time', 'message.user as from:integer',
            ...
            'pm_recipients.recipient:integer'
        );
        $table->join('users', 'message.user', 'users.id');
        $table->join('pm_recipients', 'message.id', 'pm_recipients.message');
        $table->and()->openWhere('pm_recipients.recipient', $user)->or();
        $table->closeWhere('message.user', $user);
...
//                $subTable = Jaws_ORM::getInstance()->table('pm_messages')->select('count(id):integer')->where('parent', $table->expr('message.id'));
     ...
     */
    function GetAllMessagesStatistics($user, $filters = null)
    {
        $table = Jaws_ORM::getInstance()->table('pm_messages', 'message');
