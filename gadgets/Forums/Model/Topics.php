...
class Forums_Model_Topics extends Jaws_Gadget_Model
{
    ...
    function InsertTopic($uid, $fid, $subject, $message, $published = true)
    {
        if (!$this->gadget->GetPermission('ForumPublic', $fid)) {
            return new Jaws_Error(Jaws::t('ERROR_ACCESS_DENIED'), 403, JAWS_ERROR_NOTICE);
        }
        ...
        $topic_link, $topic_subject, $topic_message, $reason = null)
    {
        ...
        $event_subject = $this::t("TOPICS_{$event_type}_NOTIFICATION_SUBJECT", $forum_title);
        $event_message = $this::t("TOPICS_{$event_type}_NOTIFICATION_MESSAGE", $lnkProfile->Get());
