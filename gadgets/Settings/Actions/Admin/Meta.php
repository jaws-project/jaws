...
class Settings_Actions_Admin_Meta extends Settings_Actions_Admin_Default
{
    ...
    function MetaSettings()
    {
        ...
        $addButton->AddEvent(ON_CLICK, "Jaws_Gadget.getInstance('Settings', 'MetaSettings').addCustomMeta();");
        $tpl->SetVariable('addButton', $addButton->Get());

        // Save Button
        $saveButton =& Piwi::CreateWidget('Button', 'save', _t('GLOBAL_SAVE'), STOCK_SAVE);
        $saveButton->AddEvent(ON_CLICK, "Jaws_Gadget.getInstance('Settings', 'MetaSettings').submitMetaForm();");
