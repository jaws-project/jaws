...
class Users_Model_Admin_Settings extends Jaws_Gadget_Model
{
    ...
    function SaveSettings($method, $anon, $repetitive, $act, $group, $recover)
    {
        ...
        $res = $res && $this->gadget->registry->set('anon_repetitive_email', $repetitive);
        $res = $res && $this->gadget->registry->set('anon_activation', $act);
        $res = $res && $this->gadget->registry->set('anon_group', (int)$group);
        $res = $res && $this->gadget->registry->set('password_recovery', $recover);
        if ($res) {
            return true;
        }
