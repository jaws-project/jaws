...
function encryptedElement(element, exponent, modulus, doCrypt, maxDigits)
{
    if (typeof(doCrypt) == 'undefined') doCrypt = true;
    if (typeof(maxDigits) == 'undefined') maxDigits = 128;
    if (!doCrypt) return true;
    setMaxDigits(maxDigits);
    var pub_key = new RSAPublicKey(exponent, modulus, maxDigits);
    element.value = encryptedString(pub_key, element.value);
    return true;
}
