...
class VAPID
{
    ...
    public static function validate(array $vapid): array
    {
        ...
        if (isset($vapid['pem'])) {
            ...
            $publicKey = PublicKey::create(Point::create(
                gmp_init(bin2hex(Jaws_JWT::base64URLDecode($jwk->get('x'))), 16),
                gmp_init(bin2hex(Jaws_JWT::base64URLDecode($jwk->get('y'))), 16)
            ));
            ...
            $vapid['privateKey'] = base64_encode(str_pad(Jaws_JWT::base64URLDecode($jwk->get('d')), 2 * self::PRIVATE_KEY_LENGTH, '0', STR_PAD_LEFT));
        }
        ...
        $publicKey = Jaws_JWT::base64URLDecode($vapid['publicKey']);
        ...
        $privateKey = Jaws_JWT::base64URLDecode($vapid['privateKey']);
    ...
    public static function getVapidHeaders(string $audience, string $subject, string $publicKey, string $privateKey, string $contentEncoding, ?int $expiration = null)
    {
        ...
        $objJWT = new Jaws_JWT(array('private' => $privateKey), 'ES256');
        $jwt = $objJWT->encode(
            array(
                'sub' => $subject,
                'aud' => $audience,
    ...
    public static function createVapidKeys(): array
    {
        ...
        return [
            'publicKey'  => Jaws_JWT::base64URLEncode($binaryPublicKey),
            'privateKey' => Jaws_JWT::base64URLEncode($binaryPrivateKey)
        ];
    }
}
